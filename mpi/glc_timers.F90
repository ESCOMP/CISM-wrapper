...
!
! !REVISION HISTORY:
!  SVN:$Id: ice_timers.F90 66 2007-05-02 16:52:51Z dbailey $
!
...
!              CICE timers are commented out.  Add timers as desired.
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
   ...
   if (srch_error /= 0) &
      call exit_glc(sigAbort, 'get_glc_timer: Exceeded maximum number of timers')
...
!-----------------------------------------------------------------------
   ...
   if (all_timers(timer_id)%in_use) then
      ...
      call exit_glc &
                 (sigAbort, 'release_glc_timer: attempt to reset undefined timer')
   endif
...
!-----------------------------------------------------------------------
   ...
   if (all_timers(timer_id)%in_use) then
      ...
      call exit_glc &
                 (sigAbort, 'glc_timer_clear: attempt to reset undefined timer')
...
!-----------------------------------------------------------------------
   ...
   if (all_timers(timer_id)%in_use) then
      ...
      call exit_glc &
                 (sigAbort, 'glc_timer_start: attempt to start undefined timer')
...
!-----------------------------------------------------------------------
   ...
   if (all_timers(timer_id)%in_use) then
      ...
      call exit_glc &
                 (sigAbort, 'glc_timer_print: attempt to print undefined timer')
