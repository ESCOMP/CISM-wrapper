...
!----------------------------------------------------------------------
   ...
   do jblock=1,nblocks_y
      ...
      if (js > ny_global) call exit_glc(sigAbort, &
            'glc: create_blocks: Bad block decomp: ny_block too large?')
      ...
      do iblock=1,nblocks_x
         ...
         if (is > nx_global) call exit_glc(sigAbort, &
            'glc: create_blocks: Bad block decomp: nx_block too large?')
            ...
            if (j_global(j,n) < 1) then
               ...
               case default
                  call exit_glc(sigAbort, &
                           'glc: create_blocks: unknown n-s bndy type')
               end select
            endif
         ...
         do i=1,nx_block
            ...
            if (i_global(i,n) < 1) then
               ...
               case default
                  call exit_glc(sigAbort, &
                           'glc: create_blocks: unknown e-w bndy type')
               end select
            endif
...
!----------------------------------------------------------------------
   ...
   if (block_id < 1 .or. block_id > nblocks_tot) then
      call exit_glc(sigAbort, 'glc: get_block: invalid block_id')
   endif
...
!----------------------------------------------------------------------
   ...
   if (block_id < 1 .or. block_id > nblocks_tot) then
      call exit_glc(sigAbort, 'glc: get_block_parameter: invalid block_id')
   endif
