!|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
 ...
 module glc_forcing_coupled

!BOP
!MODULE: glc_forcing_coupled

! !DESCRIPTION:
!  This module contains all the routines necessary for coupling glc to
!  other model components using theCCSM flux coupler.  To
...
!  WHL, May 2007: Adapted from forcing_coupled.F90 in POP 2.0
...
!
!-----------------------------------------------------------------------
!lipscomb - deleted most of the fields below, left a few as templates for glc fields
...
!
!lipscomb - fill in glc states and fluxes here
!      o  index_g2c_So_t       -- surface temperature
!
!
!    cpl6 --> glc  
!    ============
...
!
!lipscomb - Insert fields needed by glc
!     o  index_c2g_Foxx_snow   -- water flux due to snow
!     o  index_c2g_Foxx_rain   -- water flux due to rain
!
!-----------------------------------------------------------------------

!lipscomb - Insert g2c states and fluxes here
   integer(kind=int_kind) :: index_g2c_Fg_roff   ! water flux: runoff

!lipscomb - Insert c2g states and fluxes here
   integer(kind=int_kind) :: index_g2c_Sg_t        ! temperature
...
!lipscomb - replace with appropriate glc fields
   real (r8), dimension(nx_block,ny_block,max_blocks) ::  &
...
!lipscomb - Lots of mods needed for glc fields
!BOP
...
!!   real (r8), dimension(nx_block,ny_block,2,max_blocks),  &
!!      intent(out) ::   &
!!      SMF,             &!  surface momentum fluxes (wind stress)
!!      SMFT              !  surface momentum fluxes at T points

!!   real (r8), dimension(nx_block,ny_block,nt,max_blocks),  &
!!      intent(out) ::   &
...
!-----------------------------------------------------------------------
   ...
   if (nblocks /= 1) then
      call exit_glc(sigAbort,'ERROR init_coupled requires nblocks = 1')
   endif
...
!lipscomb - Commented out some POP-specific code
!-----------------------------------------------------------------------
!
...
!lipscomb - Replace with appropriate fields for glc
...
!-----------------------------------------------------------------------
      ...
      allocate (SBUFF_SUM(nx_block,ny_block,max_blocks,nsend))

!lipscomb - Replace with appropriate glc fields

      !--- determine send indices
      index_g2c_Forr_roff   = cpl_interface_contractIndex(contractR,'Fg_roff')

      !--- determine receive indices
      index_c2g_So_t     = cpl_interface_contractIndex(contractS,'Sg_t')
...
!lipscomb - Modify for glc
! Removed all arguments except ifrac

 subroutine set_coupled_forcing(IFRAC)
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   real (r8), dimension(nx_block,ny_block,max_blocks), intent(inout) ::  &
...
!lipscomb - not needed for glc?  Mostly commented out

 subroutine set_combined_forcing (FW)
...
!lipscomb - modify for glc.  Mostly commented out

!BOP
! !IROUTINE: recv_from_coupler
! !INTERFACE:

 subroutine recv_from_coupler(IFRAC)
...
!!   real (r8), dimension(nx_block,ny_block,2,max_blocks), intent(out) ::  &
!!      SMF,              &!  surface momentum fluxes (wind stress)
!!      SMFT               !  surface momentum fluxes at T points

!!   real (r8), dimension(nx_block,ny_block,nt,max_blocks), intent(out) ::  &
!!      STF,             &!  surface tracer fluxes
!!      TFW               !  tracer concentration in water flux

!!   real (r8), dimension(nx_block,ny_block,max_blocks), intent(out) ::  &
!!      SHF_QSW,          &!  penetrative solar heat flux
!!      FW,               &!  fresh water flux
!!      IFRAC,            &!  fractional ice coverage
...
!maltrud  ASSUME NBLOCKS = 1
   iblock = 1
   this_block = get_block(blocks_glc(iblock),iblock)
...
!lipscomb - Replace with glc code to unpack and distribute
!-----------------------------------------------------------------------
!
...
!lipscomb - lots of mods needed here and below
!BOP
...
!***********************************************************************
 ...
 end module glc_forcing_coupled
