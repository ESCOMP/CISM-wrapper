...
!  WHL, May 2007: Adapted from io_binary.F90 in POP 2.0
...
!-----------------------------------------------------------------------
      ...
      if (unit <= 0) then  ! no header file, assume fields are defined 
         ...
         call broadcast_scalar(hdr_error, master_task)
         if (hdr_error /= 0) call exit_glc(sigAbort,&
...
!-----------------------------------------------------------------------
   ...
   if (associated(io_field%field_i_2d)) then
      ...
      call exit_glc(sigAbort,'read_field: field not associated')
   end if
...
!-----------------------------------------------------------------------
   ...
   call scatter_global(INT2D, IOBUFI, master_task, distrb_info, &
                       field_loc, field_type)
...
!-----------------------------------------------------------------------
   ...
   call scatter_global(REAL2D, IOBUFR, master_task, distrb_info, &
                       field_loc, field_type)
...
!-----------------------------------------------------------------------
   ...
   call scatter_global(DBL2D, IOBUFD, master_task, distrb_info, &
                       field_loc, field_type)
...
!-----------------------------------------------------------------------
   ...
   call gather_global(IOBUFI, INT2D, master_task, distrb_info)
...
!-----------------------------------------------------------------------
   ...
   call gather_global(IOBUFR, REAL2D, master_task, distrb_info)
...
!-----------------------------------------------------------------------
   ...
   call gather_global(IOBUFD, DBL2D, master_task, distrb_info)
