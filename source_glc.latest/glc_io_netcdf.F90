...
!  WHL, May 2007: Adapted from io_netcdf.F90 in POP 2.0
...
!-----------------------------------------------------------------------
   ...
   if (iostat /= nf90_noerr) &
      call exit_glc(sigAbort,'error opening netCDF file for reading')
...
!-----------------------------------------------------------------------
   ...
   call broadcast_scalar(iostat, master_task)
   if (iostat /= nf90_noerr) call exit_glc(sigAbort, &
                                           'Error opening file')
...
!-----------------------------------------------------------------------
   ...
   if (attrib_error) call exit_glc(sigAbort, &
...
!-----------------------------------------------------------------------
      ...
      if (define_error) then
        ...
        call exit_glc(sigAbort, 'Error defining netCDF field')
      endif
...
!-----------------------------------------------------------------------
      ...
      call broadcast_scalar(define_error, master_task)
      if (define_error) call exit_glc(sigAbort, &
...
!-----------------------------------------------------------------------
   ...
   call broadcast_scalar(write_error, master_task)
   if (write_error) call exit_glc(sigAbort, &
      ...
      do k = 1,size(io_field%field_r_3d,dim=3)
         ...
         if (.not. read_error) &
            call scatter_global(io_field%field_r_3d(:,:,k,:), &
                                global_r_2d, master_task, distrb_info, &
      ...
      do k = 1,size(io_field%field_d_3d,dim=3)
         ...
         if (.not. read_error) &
            call scatter_global(io_field%field_d_3d(:,:,k,:), &
                                global_d_2d, master_task, distrb_info, &
                                io_field%field_loc, io_field%field_type)
      ...
      do k = 1,size(io_field%field_i_3d,dim=3)
         ...
         if (.not. read_error) &
            call scatter_global(io_field%field_i_3d(:,:,k,:), &
                                global_i_2d, master_task, distrb_info, &
                                io_field%field_loc, io_field%field_type)
...
!-----------------------------------------------------------------------
   ...
   call broadcast_scalar(read_error, master_task)
   if (read_error) &
      call exit_glc(sigAbort,'Error reading field from netCDF file')
   ...
   if (present(nftype)) then
      select case (trim(nftype))
        ...
        case ('char','CHAR','character', 'CHARACTER')
          xtype = NF90_CHAR
        case default
          call exit_glc(sigAbort,'unknown nftype') 
      end select
   else
    xtype = NF90_FLOAT
   endif
...
!-----------------------------------------------------------------------
   ...
   if (.not. supported) call exit_glc(sigAbort, &
...
!-----------------------------------------------------------------------
   ...
   if (define_error) then
       ...
       call exit_glc(sigAbort, 'Error defining nonstandard CCSM netCDF field')
   endif
