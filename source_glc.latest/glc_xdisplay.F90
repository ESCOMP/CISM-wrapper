...
!  WHL, May 2007: Adapted from xdisplay.F90 in POP 2.0

! !USES:

   use glc_kinds_mod, only: log_kind, int_kind, r8
   use glc_blocks, only: nx_block, ny_block, block
   use glc_domain_size
   use glc_domain, only:
   use glc_constants, only: delim_fmt, blank_fmt, c1, c0
   use glc_communicate, only: my_task, master_task
   use glc_io_types, only: nml_in, nml_filename, stdin, stdout
   use glc_broadcast, only: broadcast_scalar
!!   use prognostic, only: max_blocks_clinic
!!   use grid, only: 
   use glc_exit_mod, only: sigAbort, exit_glc
...
!-----------------------------------------------------------------------
   ...
   if (nml_error /= 0) then
      call exit_glc(sigAbort,'ERROR reading xdisplay_nml')
   endif
...
!-----------------------------------------------------------------------
   ...
   if (lxdisplay) call exit_glc(sigAbort, &
