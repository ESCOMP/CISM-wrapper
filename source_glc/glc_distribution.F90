...
!----------------------------------------------------------------------
   ...
   case default
      ...
      call exit_glc(sigAbort, 'glc distribution: unknown distribution type')
...
!----------------------------------------------------------------------
   ...
   if (minval(proc_tmp) < 1) then
      call exit_glc(sigAbort, 'glc: Load-balanced distribution failed')
   endif
...
!----------------------------------------------------------------------
   ...
   if (nprocs_x == 0) then
      call exit_glc(sigAbort, 'glc: Unable to find 2d processor config')
   endif
